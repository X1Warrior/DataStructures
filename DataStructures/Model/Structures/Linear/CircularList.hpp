...
template <class Type>
void CirularList<Type> :: add(Type item)
{
...
template <class Type>
void CirularList<Type> :: addAtIndex(int index, Type item)
{
    assert (index >= 0 && index <= this->size);
...
{
    assert (index >= 0 && index < this->size);
    DoubleNode<Type> * remove = findNode(index);
    DoubelNode<Type> * removedPrevious = removed->getPrevious();
